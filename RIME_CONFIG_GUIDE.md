...
A: 是的，它们共享同一个配置目录。任何一方的配置修改都会影响另一方。这正是配置共享的便利之处。

### Q: 我只想在 VoCoType 中使用特定配置怎么办？

A: Rime 的配置是全局的，无法针对不同的前端（ibus-rime vs VoCoType）使用不同配置。如果需要完全独立的配置，建议只安装 VoCoType 纯语音版。
